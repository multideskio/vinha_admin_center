...
export const churchProfileSchema = z.object({
  ...
  phone: z.string().min(1, { message: 'O celular é obrigatório.' }),
  titheDay: z.coerce.number().min(1).max(31).nullable().optional(),
  treasurerFirstName: z
    .string()
    .min(1, 'O nome do tesoureiro é obrigatório.')
    .nullable()
    .optional(),
  treasurerLastName: z
    .string()
    .min(1, 'O sobrenome do tesoureiro é obrigatório.')
    .nullable()
    .optional(),
  treasurerCpf: z
    .string()
    .min(14, 'O CPF do tesoureiro deve ter 11 dígitos.')
    .nullable()
    .optional(),
  ...addressSchema,
  ...socialLinksSchema,
...
// Tipo para configurações de notificação do usuário
