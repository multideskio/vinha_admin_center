...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const rateLimitResult = await rateLimit('igreja-dashboard', ip, 60, 60)
    if (!rateLimitResult.allowed) {
      console.error('[IGREJA_DASHBOARD_RATE_LIMIT]', {
        ip,
    ...
    if (!sessionUser) {
      ...
      console.error('[IGREJA_DASHBOARD_AUTH_ERROR]', {
        ip,
        timestamp: new Date().toISOString(),
      })
    ...
    if (!['igreja', 'church_account'].includes(sessionUser.role)) {
      console.error('[IGREJA_DASHBOARD_ROLE_ERROR]', {
    ...
    const paymentMethodsData = await db
      ...
      .where(
        startDateParam || endDateParam
          ...
          : and(eq(transactions.originChurchId, churchId), eq(transactions.status, 'approved')),
      )
    ...
    console.error('[IGREJA_DASHBOARD_ERROR]', {
