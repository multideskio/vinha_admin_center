...
export async function GET(
  ...
  if (!sessionUser || sessionUser.role !== 'manager' || sessionUser.id !== params.id) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Erro ao buscar gerente', details: error.message },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (validatedData.titheDay !== undefined)
        userUpdateData.titheDay = validatedData.titheDay;
      ...
      if (validatedData.landline !== undefined)
        profileUpdateData.landline = validatedData.landline;
      if (validatedData.cep !== undefined)
        profileUpdateData.cep = validatedData.cep;
      if (validatedData.state !== undefined)
        profileUpdateData.state = validatedData.state;
      if (validatedData.city !== undefined)
        profileUpdateData.city = validatedData.city;
    ...
    return NextResponse.json(
      { error: 'Erro ao atualizar gerente', details: error.message },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Erro ao excluir gerente', details: error.message },
      { status: 500 }
    );
  }
}
