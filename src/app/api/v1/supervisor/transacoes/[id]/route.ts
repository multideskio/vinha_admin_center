...
import { transactions as transactionsTable, gatewayConfigurations, pastorProfiles, churchProfiles } from '@/db/schema';
...
async function verifyTransactionOwnership(transactionId: string, supervisorId: string): Promise<boolean> {
...
export async function GET(request: Request, { params }: { params: { id: string }}): Promise<NextResponse> {
    ...
    try {
        ...
        if(!transaction || !transaction.gatewayTransactionId) {
            throw new ApiError(404, "Transação não encontrada no banco de dados local ou não possui ID de gateway.");
        }
