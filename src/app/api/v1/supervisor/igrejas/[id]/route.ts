...
async function verifyChurch(churchId: string) {
    ...
    if (!church || church.supervisorId !== SUPERVISOR_INIT_ID) return false;
    return true;
}
...
export async function GET(request: Request, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        if (!isAuthorized) {
            return NextResponse.json({ error: "Igreja não encontrada ou não pertence a esta supervisão." }, { status: 404 });
        }
