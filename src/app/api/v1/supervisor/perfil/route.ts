...
export async function GET(request: Request): Promise<NextResponse> {
    ...
    try {
        ...
        return NextResponse.json({ error: "Erro ao buscar perfil do supervisor", details: error.message }, { status: 500 });
    }
}
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        if (Object.keys(profileUpdateData).length > 0) {
            await tx.update(supervisorProfiles).set(profileUpdateData).where(eq(supervisorProfiles.userId, sessionUser.id));
        }
      });
      ...
      return NextResponse.json({ error: "Erro ao atualizar perfil do supervisor", details: error.message }, { status: 500 });
    }
}
