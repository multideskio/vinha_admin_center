...
if (allPassed) {
  console.log(
    colors.bold(colors.green('🎉 Todas as verificações passaram! Código pronto para commit.')),
  )
  process.exit(0)
} else {
  console.log(
    colors.bold(
      colors.red('❌ Algumas verificações falharam. Corrija os problemas antes do commit.'),
    ),
  )
  process.exit(1)
}
