...
class LegacyDataExtractor {
  ...
  async extractManagers() {
    ...
    let lastPageData = ''
    let maxPages = 4 // Limite conhecido pelo usuário
    ...
    while (hasNextPage && consecutiveEmptyPages < 2 && currentPage <= maxPages) {
      ...
      try {
        ...
        if (managers.length === 0) {
          ...
          console.log(`   📊 ${managers.length} gerentes extraídos desta página (total: ${allManagers.length})`)
        }
        ...
        const paginationInfo = await this.page.evaluate(() => {
          ...
          if (!nextButton) {
            nextButton = document.querySelector('a:contains("Próximo"), a:contains("Next"), a:contains(">")')
          }
          
          const isNextEnabled = nextButton && 
          ...
          const allPageButtons = document.querySelectorAll('.paginate_button:not(.previous):not(.next)')
          const isLastPageButton = currentPageButton && 
        ...
        console.log(`   📄 Paginação: ${paginationInfo.paginationText} | Página atual: ${paginationInfo.currentPageNumber}`)
  ...
  async extractSupervisors() {
    ...
    let consecutiveEmptyPages = 0
    let lastPageData = ''
    let maxPages = 10 // Limite razoável para supervisores
    ...
    while (hasNextPage && consecutiveEmptyPages < 2 && currentPage <= maxPages) {
      ...
      try {
        ...
        if (supervisors.length === 0) {
          ...
          console.log(`   📊 ${supervisors.length} supervisores extraídos desta página (total: ${allSupervisors.length})`)
        }
  ...
  async extractUsers() {
    ...
    let consecutiveEmptyPages = 0
    let lastPageData = ''
    let maxPages = 50 // Limite razoável para usuários (1.101 registros)
    ...
    while (hasNextPage && consecutiveEmptyPages < 2 && currentPage <= maxPages) {
      ...
      try {
        ...
        if (users.length === 0) {
          ...
          console.log(`   📊 ${users.length} usuários extraídos desta página (total: ${allUsers.length})`)
        }
