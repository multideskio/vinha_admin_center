...
class CompleteMigration {
  ...
  private loadLegacyData() {
    ...
    if (!dataPath) {
      throw new Error(
        `Nenhum arquivo de dados encontrado. Execute primeiro: npm run migrate:extract ou npm run migrate:process`,
      )
    }
    ...
    for (const gerente of this.legacyData.gerentes) {
      try {
        ...
        if (existingUser.length > 0) {
          ...
          await db.update(users).set({ welcomeSent: true }).where(eq(users.id, existingUser[0].id))
  ...
  private async validateMigration() {
    ...
    const totalWithoutProfile =
      managersWithoutProfile.length +
      supervisorsWithoutProfile.length +
      pastorsWithoutProfile.length +
      churchesWithoutProfile.length
